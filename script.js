...
const productContent = {
  'ss-letters': {
    ...
    types: ['Mirror Finish Letters', 'Brushed Steel Letters', 'Hairline Finish Letters', 'Backlit Steel Letters'],
  ...
  'gold-letters': {
    ...
    types: ['Mirror Finish Gold Letters', 'Brushed Gold SS Letters', 'Matte Gold Stainless Steel Letters', '3D Gold Stainless Steel Letters'],
    typeDescriptions: [
  ...
  'aluminum-letters': {
    ...
    types: ['Brushed Aluminium Letters', 'Powder Coated Letters', 'Painted Aluminium Letters', 'LED Aluminium Letters'],
  ...
  'acrylic-letters': {
    ...
    types: ['Flat Acrylic Letters', 'Raised Acrylic Letters', 'Acrylic with Stainless Steel Backing', 'LED Backlit Acrylic Letters'],
  ...
  'neon-board': {
    ...
    types: ['Open Neon Lettering', 'LED Flex Neon', 'Neon Logo Boards', 'Neon Acrylic Backing Boards'],
  ...
  '3d-glow': {
    ...
    types: ['Frontlit 3D Letters', 'Backlit Halo Letters', 'Front and Backlit Letters', '3D LED Box Sign Boards'],
  ...
  'pylon-board': {
    ...
    types: ['Single-Sided Pylon Signs', 'Double-Sided Pylon Signs', 'Multi-Tenant Directory Pylons', 'Illuminated Roadside Pylons'],
  ...
  'modular-signage': {
    ...
    types: ['Directional Signage', 'Room and Door Signs', 'Directory Boards', 'Suspended and Wall-Mounted Signs'],
  ...
  'acp-cladding': {
    ...
    types: ['Solid Colour ACP Cladding', 'Metallic Finish Cladding', 'Signage Fascia Cladding', 'Feature Wall and Facade Cladding'],
...
function renderProjectGallery(filter = 'All') {
  ...
  const filteredProjects = filter === 'All'
    ? projectItems
    : projectItems.filter((item) => item.category === filter || item.title.includes(filter));
...
function bindProjectFilters() {
  ...
  filters.forEach((button) => {
    button.addEventListener('click', () => {
      ...
      renderProjectGallery(button.dataset.filter || 'All');
    });
  });
}
...
function renderProductDetail() {
  ...
  const content = productContent[type] || productContent['ss-letters'];
  ...
  container.innerHTML = `
    <section class="product-introduction">
      <div class="container product-copy-narrow">
        ...
        <p>${content.intro}</p>
        ${content.introSecond ? `<p>${content.introSecond}</p>` : ''}
      </div>
    </section>
    ...
    <section class="product-definition">
      <div class="container product-definition-grid">
        <div>
          ...
          ${content.definitionSecond ? `<p>${content.definitionSecond}</p>` : ''}
          ${content.definitionThird ? `<p>${content.definitionThird}</p>` : ''}
          ${content.definitionFourth ? `<p>${content.definitionFourth}</p>` : ''}
          <p><strong>Best suited for:</strong> ${content.applications}</p>
        </div>
        <img src="${product.hero}" alt="${product.title} example">
      </div>
    </section>
    ...
    <section class="product-types">
      <div class="container">
        <div class="section-heading left">
          <span class="eyebrow">Explore the options</span>
          <h2>Types of ${product.title}</h2>
          <p>Explore options designed for different branding, finish and installation needs.</p>
        </div>
        <div class="product-type-grid">
          ${content.types.map((item, index) => `
            <article class="product-type-card">
              ...
              <h3>${item}</h3>
              <p>${content.typeDescriptions?.[index] || `${product.features[index % product.features.length]}. Designed to suit the visual and practical requirements of your space.`}</p>
              <strong>${content.applications.split(',')[index % content.applications.split(',').length].trim()}</strong>
            </article>
          `).join('')}
        </div>
      </div>
